...
int	do_move(t_data *mlx, char flag)
{
	if(flag == 'W' && check_wall(mlx, 'W'))
	{
		...
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, mlx->char_r_img, mlx->p_x * 64, (mlx->p_y - 1) * 64);
		mlx->p_y -= 1;
	}
	if(flag == 'A' && check_wall(mlx, 'A'))
	{
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, mlx->floor_img, mlx->p_x * 64, mlx->p_y * 64);
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, mlx->char_l_img, (mlx->p_x - 1) * 64, mlx->p_y * 64);
		mlx->p_x -= 1;
	}
	...
	{
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, mlx->floor_img, mlx->p_x * 64, mlx->p_y * 64);
		mlx_put_image_to_window(mlx->mlx_ptr, mlx->win_ptr, mlx->char_r_img, (mlx->p_x + 1) * 64, mlx->p_y * 64);
		mlx->p_x += 1;
	}
	return(0);
}
